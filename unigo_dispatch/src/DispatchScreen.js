...
import { collection, onSnapshot, query, doc, getDoc, deleteDoc, where, orderBy, limit } from "firebase/firestore";
import { useEffect, useState } from 'react';

function DispatchScreen() {
    const [data, setData] = useState([]);

    // get ride requests from firestore
    useEffect(() => {
        const q = query(collection(db, "rideRequests"), where("status", "==", "waiting"), orderBy("time", "asc"));
                ...
                <div style={{ flex: 1 }}>
                    ...
                    <ul>
                        {data.map((request, key) => {
                            return (
                                <div key={request.id} style={{ display: 'flex', flexDirection: 'row', justifyContent: 'center', backgroundColor: 'ButtonShadow', marginBottom: '10px' }}>
